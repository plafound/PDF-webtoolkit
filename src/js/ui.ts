...
const createFileInputHTML = (options = {}) => {
  ...
  return `
        <div id="drop-zone" class="relative flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-gray-600 rounded-xl cursor-pointer bg-red-900/50 hover:bg-red-700/50 transition-colors duration-300">
  ...
  merge: () => `
    ...
    <div id="merge-options" class="hidden mt-6">
        ...
        <div id="file-mode-panel">
            <div class="p-3 bg-red-900/30 rounded-lg border border-red-700/30 mb-3">
        ...
        <div id="page-mode-panel" class="hidden">
             ...
             <div id="page-merge-preview" class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-4 p-4 bg-red-900/30 rounded-lg border border-red-700/30 min-h-[200px]"></div>
        </div>
  ...
  'add-watermark': () => `
    ...
    <div id="watermark-options" class="hidden mt-6 space-y-4">
        <div class="mb-4">
            ...
            <div id="watermark-preview-container" class="relative w-full max-w-xl mx-auto bg-gray-900 border border-gray-600 rounded-lg overflow-hidden">
                <canvas id="pdf-canvas" class="block w-full h-auto"></canvas>
                <canvas id="watermark-canvas" class="absolute inset-0 w-full h-full pointer-events-none"></canvas>
            </div>
        </div>
        ...
        <div id="image-watermark-options" class="hidden space-y-4">
            ...
            <div>
                <label for="size-image" class="block mb-2 text-sm font-medium text-gray-300">Size (<span id="size-value-image">0.5</span>)</label>
                <input type="range" id="size-image" value="0.5" min="0.1" max="1.5" step="0.05" class="w-full">
            </div>
        </div>
