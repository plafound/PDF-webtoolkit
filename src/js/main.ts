...
const init = () => {
  ...
  if (__SIMPLE_MODE__) {
    const hideBrandingSections = () => {
      ...
      document.title = 'PDF-Web Toolkit';
  ...
  categories.forEach((category) => {
    ...
    title.className = 'text-xl font-bold text-orange-400 mb-4 mt-8 first:mt-0';
    ...
    category.tools.forEach((tool) => {
      ...
      toolCard.className =
        'tool-card bg-red-800/50 rounded-xl p-4 cursor-pointer flex flex-col items-center justify-center text-center';
      ...
      if (tool.subtitle) {
        ...
        toolSubtitle.className = 'text-xs text-gray-200 mt-1 px-2';
