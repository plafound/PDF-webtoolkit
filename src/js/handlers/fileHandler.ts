...
import { renderPdfPreview, setupTextWatermarkPreview, setupImageWatermarkLoader, setupImageWatermarkPreview } from '../logic/preview-watermark.js';
...
async function handleSinglePdfUpload(toolId, file) {
  ...
  try {
    ...
    if (toolId === 'add-watermark') {
      renderPdfPreview(pdfBytes as ArrayBuffer);
      setupTextWatermarkPreview();
      setupImageWatermarkLoader();
      setupImageWatermarkPreview();
    }
